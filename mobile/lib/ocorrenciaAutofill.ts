...
export function suggestFromTitulo(titulo: string): OcorrenciaSuggestions {
  ...
  let prioridade: PrioridadeOcorrencia;
  if (
    includesAny(t, [
      'incendio',
      'fogo',
  ...
  let categoria: CategoriaOcorrencia;
  if (
    includesAny(t, [
      'fuga',
      'agua',
    ...
    includesAny(t, ['disjuntor', 'luz', 'tomada', 'interruptor', 'curto', 'eletricidade', 'eletrico'])
  ) {
    categoria = 'eletricidade';
  } else if (includesAny(t, ['elevador', 'cabine', 'porta do elevador'])) {
    categoria = 'elevador';
  } else if (
    includesAny(t, ['extintor', 'alarme', 'incendio', 'fumo', 'fumaca', 'detetor', 'detector'])
  ) {
    categoria = 'seguranca_incendio';
  } else if (
    includesAny(t, ['escadas', 'fachada', 'parede', 'teto', 'tecto', 'chao', 'piso', 'rachadura'])
  ) {
    categoria = 'estrutural';
  } else if (
    includesAny(t, ['porta', 'corrimao', 'garagem', 'hall', 'entrada', 'jardim', 'portao'])
  ) {
